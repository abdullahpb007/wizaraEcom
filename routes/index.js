...
router.get('/warranty', (req ,res , next) =>{
      ...
      if(!req.params.page){
          ...
          .then(([results, menu]) => {
              ...
              res.render(`${config.themeViews}warranty`, {
                  title: `${config.cartTitle} - Products`,
                  theme: config.theme,
    ...
    router.get('/resources', (req ,res , next) =>{
          ...
          if(!req.params.page){
              ...
              .then(([results, menu]) => {
                  ...
                  res.render(`${config.themeViews}resources`, {
                      title: `${config.cartTitle} - Products`,
                      theme: config.theme,
                      results: results.data,
                      session: req.session,
        ...
        router.get('/faqs', (req ,res , next) =>{
              ...
              if(!req.params.page){
                  ...
                  .then(([results, menu]) => {
                      ...
                      res.render(`${config.themeViews}faqs`, {
                          title: `${config.cartTitle} - Products`,
                          theme: config.theme,
                          results: results.data,
                          session: req.session,
...
router.get('/theme2', (req ,res , next) =>{
      ...
      if(!req.params.page){
          ...
          .then(([results, menu]) => {
              ...
              res.render(`${config.themeViews}theme2`, {
                  title: `${config.cartTitle} - Products`,
                  theme: config.theme,
                  results: results.data,
                  session: req.session,
    ...
    router.get('/grape', (req ,res , next) =>{
          ...
          if(!req.params.page){
              ...
              .then(([results, menu]) => {
                  ...
                  res.render(`${config.themeViews}grape`, {
                      title: `${config.cartTitle} - Products`,
                      theme: config.theme,
                      results: results.data,
                      session: req.session,
        ...
        router.post('/grape', (req ,res , next) =>{
                  const db = req.app.db;
                  let doc = {
                      'gjs-assets':req.body['gjs-assets'],
                      'gjs-css':req.body['gjs-css'],
                      'gjs-styles':req.body['gjs-styles'],
                      'gjs-html':req.body['gjs-html'],
                      'gjs-components':req.body['gjs-component'],
                      'Ident':2
                  };

                  db.templates.insert(doc, (err, newDoc) => {
                      if(err){
                          console.log(colors.red('Error inserting document: ' + err));
                          // Need to work with keeping the current stuff
        ...
        router.get('/grape/load', (req ,res , next) =>{
          const db = req.app.db;
          const id = "5d17b67ca834840f88b2c123";

          db.templates.findOne({ Ident: 2 }, (err, temp) => {
            if(err){
              console.log(err);
            }else{
              res.send(temp)
    ...
    // });
      ...
      if(!req.params.page){
          ...
          .then(([results, menu]) => {
              ...
              res.render(`${config.themeViews}prod`, {
                  title: `${config.cartTitle} - Products`,
                  theme: config.theme,
                  results: results.data,
                  ident:"all",
...
// show an individual group
    router.get('/prod/:id', (req, res) => {
        ...
        if(!req.params.page){
            Promise.all([
                getData(req, 1, {productCategory:req.params.id}),
                getMenu(db)
            ])
            .then(([results, menu]) => {
              // for setting images
              ...
              if(results.data){
                for(var key in results.data){
                   seperator = results.data[key].productCategory;
                }
              }
              // for showing individual group
              if(results.data.length >= 2){
                for(var i=0;i<results.data.length;i++){
                  for(var j=i+1;j<results.data.length;j++){
                    if(results.data[i].productGroup === results.data[j].productGroup){
                      results.data.splice(j, 1);
                    }
                  }
                }
              }
                ...
                res.render(`${config.themeViews}group`, {
                    title: `${config.cartTitle} - Products`,
                    theme: config.theme,
                    results: results.data,
                    cate: param,
    ...
    router.get('/prod/:category/:group', (req, res) => {
        let db = req.app.db;
        let config = req.app.config;
        let numberProducts = config.productsPerPage ? config.productsPerPage : 6;

            Promise.all([
                getData(req, 1, {productCategory:req.params.category,productGroup:req.params.group}),
                getMenu(db)
            ])
            .then(([results, menu]) => {
              var seperator;
